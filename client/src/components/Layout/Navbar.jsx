    ...
    <>
      <header className='border bg-background flex items-center justify-between p-4  shadow-lg md:pb-4 fixed w-full z-10 top-0'>
        ...
        <div className=' hidden text-center border-2 rounded-xl  w-full md:flex md:w-2/5 lg:flex'>
          <input
            className='bg-grey-lightest focus:border-orange p-2 rounded-xl shadow-inner w-full'
            placeholder='Search'
            type='text'
          />
          <button
            type='submit'
            className=' text-2xl font-extrabold p-2  text-black hover:text-gray-800'
          >
            <BsSearch />
          </button>
        </div>
        <div>
          <nav className='flex gap-2'>
            <ul className=' hidden md:flex md:items-center '>
              {AllLinks.map((link) => (
                <li
                  className='text-text md:ml-2 md:p-2 rounded-xl  hover:bg-gray-100  '
                  key={link.name}
                >
                  {" "}
                  {/* Add key */}
