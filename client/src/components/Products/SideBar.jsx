...
const handleSubMenuClick = (index) => {
  if (openSubMenuIndex === index) {
    ...
    return (
        <>
        <div className='sm:hidden fixed inset-x-0 bg-black/10 inset-y-0 'onClick={closeModal}></div>
        <div className="fixed flex flex-col bottom-0 inset-x-0   justify-end bg-white sm:static sm:w-full">
            <div className='ml-4 mr-4 sm:ml-0 sm:mr-0'>
                <div className=' ml-1 sm:ml-2 sm:my-2 sm:mb-0 flex flex-row'>
                    {/* ... */}
                    <span className='py-1 text-lg font-bold mr-1 hover:text-blue-600'><BsFilter/></span><span className=' font-bold text-lg hover:text-blue-600' > Filter</span>
                </div>
                <ul className="pt-2 ml-3">
                    {Menus.map((Menu, index) => (
                        <>
                            ...
                            >
                                {/* ... */}
                                {Menu.icon ? Menu.icon : <MdOutlineDashboard className=' text-green-600'/>}
